...
drv_mac80211_setup() {
	...
	sleep 2
	[[ $(pidof log_daemon) ]] || {
		touch $(cat /etc/pineapple/pineap_log_location)pineap.log
		log_daemon $(cat /etc/pineapple/pineap_log_location)pineap.log 30 &
		sleep 2
		while read mac; do
			[[ "$mac" == "" ]] || {
				/usr/bin/pineapple/uds_send /var/run/log_daemon.sock "track:$mac"
			}
		done < /etc/pineapple/tracking_list
	}
	pineapple init_karma
